  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-brand-bg px-3 py-6 text-brand-text sm:px-6 sm:py-10">
      ...
      <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className="relative w-full max-w-[29rem] rounded-[2rem] border border-brand-border bg-brand-surface/95 px-5 py-8 shadow-[0_24px_70px_rgba(0,0,0,.28)] backdrop-blur-xl sm:rounded-[2.5rem] sm:px-8 sm:py-10">
        ...
        <div className="mt-8">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-accent/25 bg-brand-accent/[.08] px-3 py-1.5 text-[11px] font-semibold text-brand-accent"><Sparkles size={13} aria-hidden="true" /> Join 1,000+ plugged-in users</span>
          <h1 className="mt-5 text-[clamp(2.25rem,9vw,3.25rem)] font-black leading-[.98] tracking-[-.055em]">Create your <span className="text-brand-accent">account.</span></h1>
          <p className="mt-3 text-[15px] leading-6 text-brand-text-secondary">Build your digital presence, keep your work organised and sell when you are ready.</p>
        </div>
        <div className="mt-7"><SignUp signInUrl="/login" forceRedirectUrl="/dashboard" appearance={clerkAppearance} /></div>
        <p className="mt-6 text-center text-sm text-brand-text-secondary">Already have an account? <Link to="/login" className="font-bold text-brand-text underline decoration-brand-accent/50 underline-offset-4 hover:text-brand-accent">Log in</Link></p>
        <p className="mt-5 flex items-center justify-center gap-1.5 text-[11px] text-brand-text-secondary"><LockKeyhole size={13} aria-hidden="true" /> Secure account creation powered by Clerk</p>
      </motion.section>
    </main>
  );
}
