...
const categories = ["All", "Courses", "Templates", "Design", "Video", "Comics", "Social media", "Business", "Creator tools"];
...
export default function Marketplace() {
  ...
  const [deliveryChoices, setDeliveryChoices] = useState<any[] | null>(null);
  ...
  const openDelivery = async (orderId: string) => {
    ...
    try {
      ...
      const choices = Array.isArray(payload.deliveries) && payload.deliveries.length
        ? payload.deliveries
        : payload.deliveryUrl ? [{ deliveryUrl: payload.deliveryUrl, label: payload.deliveryLabel || "Open product", kind: "link" }] : [];
      if (!choices.length) throw new Error("Your product has no active delivery files or links yet.");
      if (choices.length === 1) window.location.assign(choices[0].deliveryUrl);
      else setDeliveryChoices(choices);
    } catch (error: any) {
