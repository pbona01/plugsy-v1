    ...
    <main className="min-h-screen bg-brand-bg px-4 pb-28 pt-28 text-brand-text sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-7xl">
        <section className="rounded-[1.75rem] border border-brand-border bg-brand-surface p-4 shadow-xl sm:p-5">
          ...
          <div className="mt-5 inline-flex w-full max-w-md rounded-2xl border border-brand-border bg-brand-bg p-1.5"><button onClick={() => setMode("buy")} className={`flex h-11 flex-1 items-center justify-center gap-2 rounded-xl text-xs font-black uppercase tracking-wider transition ${mode === "buy" ? "bg-brand-text text-brand-bg shadow-lg" : "text-brand-text-secondary hover:text-brand-text"}`}><Search size={15} /> Buy</button><button onClick={openSeller} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-xl text-xs font-black uppercase tracking-wider text-brand-text-secondary transition hover:bg-brand-accent/10 hover:text-brand-accent"><Store size={15} /> Start selling</button></div>
        </section>
        <p className="mt-4 rounded-xl border border-amber-500/25 bg-amber-500/[.07] px-4 py-2.5 text-xs leading-5 text-brand-text-secondary">Marketplace preview: you can explore products and prepare listings now. Real purchases will open after final launch checks.</p>
