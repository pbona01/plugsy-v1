...
async function handleValidate(req, res) {
  ...
  try {
    const { code, userId } = req.body || JSON.parse(req.body)
    ...
    if (profile.clerk_id === userId) return res.status(200).json({ valid: false, message: "You cannot use your own purchase code" })
