...
The deployed service worker is `/sw.js`; it is generated by the Vite PWA build,
imports the OneSignal and badge helpers, and must be reachable at
`https://www.plugsy.ng/sw.js` as JavaScript rather than the SPA HTML fallback.
Do not register another root-scope service worker from application code.
