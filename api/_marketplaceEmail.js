...
export function buildMarketplaceEmail(job) {
  ...
  const copy = job.kind === 'refund' ? `${amount} has been returned to your Plugsy wallet. Product access has been revoked.` : job.kind === 'seller_release' ? `${amount} is now available in your Plugsy wallet after buyer protection and Processing Payout.` : `Your purchase of ${title} for ${amount} is available in My library. You have 10 hours from purchase to report a genuine product issue.`;
